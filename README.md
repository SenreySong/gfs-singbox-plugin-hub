...
- 节点同时通过主规则和额外条件并实际加入分组后才算命中；同一个节点可加入多个分组。
- 分组规则只使用策略组 tag，不再维护重复的名称字段。
- 面板使用固定规则头部和分层表单网格，避免列宽错乱与横向滚动。
- 插件下载地址指向不可变版本标签，避免更新元数据后仍命中旧脚本缓存。
- 启用 Other 组时，收纳未实际进入任何分组的节点，包括命中主规则但被所有对应额外条件排除的节点。
- 可把生成的策略组插入到未隐藏策略组中。
...
- 保留 TUN `auto_redirect` 及 `auto_redirect_tproxy_mark`；Android 完整重定向需要图形客户端 root 服务或 root shell，并启用 `auto_route`。插件不自动开启该功能，不覆盖不同平台的默认标记；仍需自行满足平台条件并避免与 `route.default_mark` / 拨号 `routing_mark` 冲突。
